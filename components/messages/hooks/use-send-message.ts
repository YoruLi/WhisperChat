...
const sendMessageToChat = async ({
  ...
  try {
    ...
    const { created_chat, inserted_message } = data as {
      created_chat: Chat;
      inserted_message: MessageData;
    };
    const last_message = {
      ...inserted_message,
      created_at: inserted_message
        ? new Date(inserted_message.created_at)
        : null,
    };
...

    {
      onMutate: (data) => {
        queryClient.setQueryData(
          ["messages"],
          (
            oldData: InfiniteMessages<MessageData> | undefined
          ): InfiniteMessages<MessageData> => {
            const { pages } = oldData || { pages: [] };
            const lastPage = (pages && pages[pages?.length - 1]) || [];
            const matchingMessage = pages
              ?.map((page) =>
                page.find((item) => item.profile_id === data.sender_profile_id)
              )
              .filter(Boolean); // Filtrar para eliminar los valores undefined
            const image =
              data?.files && data.files.length !== 0
                ? URL.createObjectURL(data.files[0])
                : null;

            const newMessage = matchingMessage && {
              ...matchingMessage[0],
