...
export default function CreatePostModal({ session }: { session: Session | null }) {
    ...
    return (
        ...
        >
            <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="bg-[#181920] fixed bottom-0 border-l border-slate-700  lg:right-0  w-full lg:w-[27.5rem] flex  flex-col lg:bottom-0  z-[31] h-full "
            >
                ...
                >
                    <div className="overflow-y-auto max-h-[calc(100vh-145px)] scrollbar ">
                        <CreatePost in_reply_status_id={parent_post} replyPost={true} placeholder={placeholder ?? ""} session={session} />
                    </div>
                </div>
            </motion.div>
        </div>
